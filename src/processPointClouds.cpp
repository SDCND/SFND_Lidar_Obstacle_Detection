...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
	...
	typename pcl::PointCloud<PointT>::Ptr cloudRemovedNear(new pcl::PointCloud<PointT>());
	pcl::CropBox<PointT> np;  // near points

	np.setMin(Eigen::Vector4f(-1.5, -1.7,   -1, 1));
	np.setMax(Eigen::Vector4f( 2.6,  1.7, -0.4, 1));
	np.setInputCloud(cloudDownsampled);
	np.setNegative(true);
	np.filter(*cloudRemovedNear);

	typename pcl::PointCloud<PointT>::Ptr cloudRegion(new pcl::PointCloud<PointT>());
	pcl::CropBox<PointT> roi;  // region of interest
	
	roi.setMin(minPoint);
	roi.setMax(maxPoint);
	roi.setInputCloud(cloudRemovedNear);
